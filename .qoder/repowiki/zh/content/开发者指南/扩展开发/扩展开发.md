...
- 阶段配置：StageConfig 提供 s01-s17 与 sFull 的能力开关与工具集
...
参考路径：
- 工具元数据构造：[StageConfig.java:703-710](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L703-L710)
...
### 新增学习阶段（基于 S01-S17 模板）
目标：创建一个新阶段（例如 S18），复用现有阶段模板，组合所需工具与能力开关。

步骤概览：
1. 在 StageConfig 中新增阶段构建方法（如 s18()），继承已有阶段工具集并叠加新功能。
2. 新建阶段入口类（如 S18xxx），调用 Launcher.launch(StageConfig.s18())。
3. 如需启用高级特性（权限、钩子、压缩、记忆、后台、定时、团队、自治、worktree、MCP、工作流、目标循环），在 StageConfig 中设置相应开关。
...
参考路径：
- 阶段构建示例：[StageConfig.java:301-532](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L301-L532)
...
**章节来源**
- [StageConfig.java:301-532](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L301-L532)
- [S01AgentLoop.java:1-12](file://src/main/java/com/learnclaudecode/agents/S01AgentLoop.java#L1-L12)
- [S02ToolUse.java:1-13](file://src/main/java/com/learnclaudecode/agents/S02ToolUse.java#L1-L13)
- [Launcher.java:18-26](file://src/main/java/com/learnclaudecode/agents/Launcher.java#L18-L26)
