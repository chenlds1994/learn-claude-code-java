...
S13["S13AgentTeams.java"] --> Launcher
Launcher --> AppContext["AppContext.java"]
AppContext --> Runtime["AgentRuntime.java"]
Runtime --> Tools["CommandTools / TodoManager / SkillLoader / TaskManager / BackgroundManager / MessageBus / TeammateManager / WorktreeManager / PermissionManager / HookManager / MemoryStore / CronScheduler / McpClient / WorkflowEngine / GoalController"]
...
- [S13AgentTeams.java:1-24](file://src/main/java/com/learnclaudecode/agents/S13AgentTeams.java#L1-L24)
- [Launcher.java:1-28](file://src/main/java/com/learnclaudecode/agents/Launcher.java#L1-L28)
- [AppContext.java:1-91](file://src/main/java/com/learnclaudecode/agents/AppContext.java#L1-L91)
- [AgentRuntime.java:1-498](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L1-L498)
...
## 核心组件
- 阶段入口类（如 S01AgentLoop、S02ToolUse、S13AgentTeams、SFull）：极薄，仅调用 Launcher.launch(StageConfig.xxx())
- Launcher：统一启动器，创建应用上下文并运行 REPL
- StageConfig：阶段配置记录，定义 prompt、工具集、能力开关（Todo、压缩、后台、收件箱、子代理可写、自治队友、worktree、权限、钩子、记忆、定时调度、MCP、工作流、目标循环）
- AppContext：装配所有共享服务（环境、路径、命令、任务、后台、团队、工作区隔离、技能加载、压缩服务，以及权限、钩子、记忆、定时调度、MCP、工作流、目标循环等扩展管理器），并返回 AgentRuntime
- AgentRuntime：统一执行循环，处理用户输入、模型调用、工具调度、消息历史、自动/手动压缩、后台结果注入、团队收件箱轮询、权限确认、钩子触发、记忆检索、定时任务与 MCP/工作流/目标循环等
...
- [AppContext.java:35-91](file://src/main/java/com/learnclaudecode/agents/AppContext.java#L35-L91)
...
- 作用：定义当前阶段的 system prompt、工具集、能力开关（enableTodoNag、enableCompression、enableBackground、enableInbox、subagentWritable、autonomousTeammates、enablePermission、enableHooks、enableMemory、enableCron、enableMcp、enableWorkflow、enableGoalLoop）
- 关键方法：
  ...
  - s01()..s17()、sFull()：预置的阶段配置构建器，体现能力逐步开放
...
- [StageConfig.java:52-57](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L52-L57)
...
### AppContext：装配器
- 作用：集中创建共享服务（环境、路径、命令、任务、后台、团队、工作区隔离、技能加载、压缩服务，以及权限、钩子、记忆、定时调度、MCP、工作流、目标循环等扩展管理器），并组装 AgentRuntime
...
- 关键逻辑：
  ...
  - agentLoop(messages, config)：单轮任务主循环，包含自动/手动压缩、后台结果注入、团队收件箱轮询、工具调度（含权限确认、钩子触发、记忆检索、MCP 工具代理、工作流与目标循环）、消息历史更新
...
### 阶段入口类与集成方式
- 每个阶段入口类（S01、S02、...、S17、SFull）仅一行：Launcher.launch(StageConfig.xxx())
...
- AppContext 依赖多个服务（命令、任务、后台、团队、工作区隔离、技能加载、压缩，以及权限、钩子、记忆、定时调度、MCP、工作流、目标循环等）
...
AR --> PM["PermissionManager"]
AR --> HM["HookManager"]
AR --> MS["MemoryStore"]
AR --> CR["CronScheduler"]
AR --> MC["McpClient"]
AR --> WE["WorkflowEngine"]
AR --> GC["GoalController"]
```
...
- [AgentRuntime.java:501-538](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L501-L538)
...
- 决定是否需要开启 Todo、压缩、后台、收件箱、子代理可写、自治队友、权限、钩子、记忆、定时调度、MCP、工作流、目标循环等开关
...
**章节来源**
- [StageConfig.java:64-78](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L64-L78)
- [StageConfig.java:703-725](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L703-L725)
