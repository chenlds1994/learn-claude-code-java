...
图表来源
- [AgentRuntime.java:137-406](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L137-L406)
...
- 阶段配置（StageConfig）：定义不同教学阶段的工具集合与 system prompt 模板；s07 引入 load_skill 工具，并将 ${SKILLS} 占位符替换为技能摘要。
...
- [AgentRuntime.java:287-406](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L287-L406)
...
### 运行时集成与工具映射
- 阶段配置 s07：新增 load_skill 工具，其 schema 仅要求 name 参数；systemTemplate 中包含 ${SKILLS} 占位符。
...
### 启动入口与示例运行
- S07SkillLoading：最小化入口，选择 s07 阶段配置并通过 Launcher 启动统一运行时。
- Launcher：统一入口，创建 AppContext 并调用 runtime().runRepl(config)。
- AppContext：装配 SkillLoader 等服务，并注入 AgentRuntime。

章节来源
- [S07SkillLoading.java:1-13](file://src/main/java/com/learnclaudecode/agents/S07SkillLoading.java#L1-L13)
