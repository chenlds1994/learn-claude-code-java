...
- [StageConfig.java:533-571](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L533-L571)
...
- 统一启动器与阶段配置
  ...
  - StageConfig 现以 s13 一个阶段合并开启原 s09-s12 的全部能力（inbox 通信、shutdown_request/plan_approval 协议、claim_task/idle 自治、worktree_* 隔离）；前端组件沿用旧编号 s09-s12 分别演示各子能力
- 文件型消息总线
...
- 后端支撑
  - StageConfig.s13() 启用 inbox 并暴露 spawn_teammate/send_message/read_inbox/broadcast 等工具（s13 合并了原 s09-s12 四个阶段）
...
- 后端支撑
  - StageConfig.s13() 同时包含 shutdown_request/plan_approval 协议工具（原 s10 能力已并入 s13）
...
- 后端支撑
  - StageConfig.s13() 还包含 claim_task/idle 与任务工具（原 s11 自治能力已并入 s13）
...
- 后端支撑
  - StageConfig.s13() 还包含 worktree_create/list/remove/events 工具（原 s12 能力已并入 s13）
...
- 协作链路
  - StageConfig.s13 暴露 inbox 通信、协议、自治与 worktree 全套工具 -> MessageBus.send/readInbox/broadcast
- 自治链路
  - StageConfig.s13 注入任务工具与 claim/idle -> 多 Agent 自行 poll/claim/work
- 隔离链路
  - StageConfig.s13 注入 worktree_* 工具 -> WorktreeManager.create/list/remove/recentEvents -> 事件落盘

```mermaid
graph LR
S13["S13AgentTeams 入口"] --> L["Launcher"]
L --> CFG["StageConfig"]
CFG --> MB["MessageBus"]
...
- 上下文与内存
  - 通过 inbox 与任务板解耦，减少进程间共享状态；建议对长对话启用上下文压缩（s08）以降低 LLM 调用成本
- 可扩展性
