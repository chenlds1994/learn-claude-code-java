...
## 简介
本文件面向“阶段专用可视化组件”的综合文档，目标是系统化说明 S01-S12 各阶段（前端组件编号）的可视化实现、交互设计与数据绑定机制。注：前端组件编号沿用旧 12 阶段映射，后端已重构为 17 阶段（如前端 s03-todo-write.tsx 对应后端 S05TodoWrite、s05-skill-loading.tsx 对应 S07SkillLoading、s07-task-system.tsx 对应 S10TaskSystem、s09-s12 四个组件合并对应 S13AgentTeams）。内容覆盖：
- Agent 循环演示（S01）
- 工具调度可视化（S02）
- 任务管理界面（S07）
- 团队与协议协作（S09/S10）
