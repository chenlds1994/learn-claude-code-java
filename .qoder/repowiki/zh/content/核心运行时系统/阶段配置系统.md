...
- [S05TodoWrite.java:1-13](file://src/main/java/com/learnclaudecode/agents/S05TodoWrite.java#L1-L13)
...
## 核心组件
- StageConfig：记录当前阶段的 system prompt 模板、工具列表、以及 enableCompression、enableBackground、enableInbox、subagentWritable、autonomousTeammates、enableTodoNag、enablePermission、enableHooks、enableMemory、enableCron、enableMcp、enableWorkflow、enableGoalLoop 等功能开关；提供 baseTools() 默认工具集与各阶段工厂方法 s01-s17、sFull。
...
- 数据模型
  ...
  - enablePermission：是否启用权限管控（s03 引入）。
  - enableHooks：是否启用钩子系统（s04 引入）。
  - enableMemory：是否启用持久记忆（s09 引入）。
  - enableCron：是否启用定时调度（s12 引入）。
  - enableMcp：是否启用 MCP 插件（s14 引入）。
  - enableWorkflow：是否启用工作流运行时（s16 引入）。
  - enableGoalLoop：是否启用目标循环（s17 引入）。
...
- 关键方法
  ...
  - s01-s17、sFull：各阶段工厂方法，组合不同的 tools 与开关。
  - tool(name, description, schema)：构造 messages API 兼容的工具定义。
  - dedupe(tools)：合并阶段时按工具名去重，保留最后一次定义。

- 阶段差异化要点
  - s01：仅 bash，最小闭环（Agent 循环）。
  - s02：加入文件读写编辑工具。
  - s03：权限系统（permission_list/set），开启 enablePermission。
  - s04：钩子系统（hook_register/list/remove），开启 enableHooks。
  - s05：引入 todo，开启 enableTodoNag。
  - s06：引入 task（子代理），控制 subagentWritable。
  - s07：引入 load_skill，结合 ${SKILLS} 动态能力加载。
  - s08：引入 compact，开启 enableCompression。
  - s09：记忆系统（memory_add/search/delete/list），开启 enableMemory。
  - s10：任务系统（task_create/update/list/get）。
  - s11：后台任务（background_run/check），开启 enableBackground。
  - s12：定时调度（schedule_cron/list_crons/cancel_cron），开启 enableCron。
  - s13：多 Agent 协作合并阶段：团队通信（spawn_teammate/send_message/read_inbox/broadcast）、团队协议（shutdown_request/plan_approval）、自治队友（claim_task/idle，开启 autonomousTeammates）与 worktree 隔离（create/list/remove/events），开启 enableInbox。
  - s14：MCP 插件（connect_mcp 及动态发现的 mcp__server__tool），开启 enableMcp。
  - s15：集成 Harness，合并 s01-s14 全部工具与开关。
  - s16：工作流运行时（workflow），开启 enableWorkflow。
  - s17：目标循环（goal_set/status/clear），开启 enableGoalLoop。
