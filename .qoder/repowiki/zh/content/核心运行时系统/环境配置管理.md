...
## 更新摘要
**所做更改**
- 更新了环境变量清单，添加了 ANTHROPIC_BASE_URL 的配置说明
- 新增了 BigModel 兼容服务的配置示例
- 更新了 .env 文件的配置说明和最佳实践
- 增强了不同环境配置差异的示例
- 完善了配置验证和错误处理机制的说明
...
**更新** 本项目现已支持 Anthropic 兼容服务，包括 BigModel 等第三方提供商，通过 ANTHROPIC_BASE_URL 配置实现灵活的服务端点切换。
...
**更新** 现在项目支持多种 Anthropic 兼容服务提供商，通过灵活的 BASE_URL 配置可以轻松切换到不同的后端服务。
...
- MODEL_ID（必填）
  - 用途：指定要使用的模型标识。
  - 默认：无（缺失即报错）。
  - 示例：glm-4.7、claude-3-sonnet-20240229 等。
...
- ANTHROPIC_BASE_URL（可选）
  ...
  - 示例：https://open.bigmodel.cn/api/anthropic（BigModel 服务）。
- 工作目录
  - 来源：当前进程工作目录的绝对路径。
  - 用途：作为所有工作区文件的根目录。
...
**更新** 项目现已支持 BigModel 等第三方 Anthropic 兼容服务，通过配置 ANTHROPIC_BASE_URL 即可轻松切换后端。

### 不同环境的配置差异示例
- 开发环境
  - 在项目根目录放置 .env 文件，设置 MODEL_ID、ANTHROPIC_API_KEY、ANTHROPIC_BASE_URL。
  - 本地 IDE 运行时可通过运行配置覆盖系统环境变量，实现更高优先级。
  - 推荐使用 BigModel 或其他第三方服务进行测试，成本更低且易于获取。
- 测试环境
  - 建议使用独立的 CI 环境变量注入，避免提交敏感信息。
  - 可使用不同的 BASE_URL 指向沙箱或测试端点。
  - 对于 BigModel 等服务，可使用其测试环境端点。
- 生产环境
  - 必须通过平台级环境变量注入，严禁将密钥写入代码仓库。
  - 建议启用更严格的监控与告警，并对网络异常进行重试与降级。
  - 根据实际服务需求选择合适的模型和服务提供商。

### 配置验证、默认值与错误处理机制
- 验证
  - 必填项缺失立即抛出异常，确保启动失败早于业务逻辑执行。
  - 对 ANTHROPIC_BASE_URL 进行基本格式验证。
- 默认值
  - API Key 与 Base URL 提供合理默认，便于最小化配置即可运行。
  - 默认使用官方 Anthropic 服务，确保开箱即用。
- 错误处理
  - 网络异常统一转换为业务异常，附带上下文信息，便于上层捕获与记录。
  - 对第三方服务的兼容性错误提供详细的错误信息。
...
- 优先级与覆盖
  ...
  - 支持通过命令行参数或 IDE 运行配置覆盖环境变量。
- 安全建议
  - 不要将真实密钥提交到代码仓库；使用 .env 仅用于本地开发。
  - 在生产环境使用平台提供的密钥管理服务注入环境变量。
  - 定期轮换 API Key，特别是使用第三方服务时。
- 可观测性
  - 对配置加载与网络调用添加结构化日志，便于问题定位。
  - 记录使用的模型 ID 和服务端点，便于问题追踪。
- 扩展性
  - 如需新增配置项，建议在 EnvConfig 中集中管理，并提供清晰的默认值与校验逻辑。
  - 支持未来添加更多第三方服务提供商的配置选项。

**更新** 建议在使用第三方服务时，明确记录服务端的特性差异和限制，以便更好地适配不同提供商。
...
- OpenRouter 服务配置
  ...
  ```
- 其他兼容服务
  - 任何支持 Anthropic Messages API 的服务都可以使用相同的配置方式。
  - 只需修改 MODEL_ID 和 ANTHROPIC_BASE_URL 即可切换服务。
