...
- 每个阶段通过静态工厂方法返回不同的 StageConfig，控制：
  ...
  - tools：逐步开放文件/命令、权限、钩子、Todo、子代理、技能加载、压缩、记忆、任务、后台、定时调度、团队、worktree、MCP、工作流、目标循环等能力
  - 开关：enableTodoNag、enableCompression、enableBackground、enableInbox、subagentWritable、autonomousTeammates、enablePermission、enableHooks、enableMemory、enableCron、enableMcp、enableWorkflow、enableGoalLoop
- sFull 合并所有阶段能力，形成完整视图。
...
- 新增工具
  ...
  - 参考路径：[StageConfig.tool(...):703-710](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L703-L710)、[AgentRuntime 工具分发:189-234](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L189-L234)。

- 自定义阶段
  - 在 StageConfig 中新增静态工厂方法，组合所需 tools 与开关，并编写合适的 systemTemplate。
  - 参考路径：[s01-s17 与 sFull:301-671](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L301-L671)。
...
- 启动指定阶段
  - 通过 S01..S17 或 SFull 的 main 调用 Launcher.launch(StageConfig.xxx())。
