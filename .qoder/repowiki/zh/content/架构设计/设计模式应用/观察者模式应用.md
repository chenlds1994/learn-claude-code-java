...
- agents 包提供阶段配置与入口：
  - StageConfig：不同阶段的工具集与提示词配置，包含 s13 的团队与协议能力。
  - S13AgentTeams：演示多 Agent 协作与团队协议的启动入口。
- common 包提供通用工具：
...
- 阶段配置 StageConfig
  - 职责：为不同教学阶段暴露不同的工具集与提示词；s13 开启团队与协议能力。
- JSON 工具 JsonUtils
...
- 阶段配置与入口
  - s13 团队协作：StageConfig.s13 暴露 spawn/list/send/read/broadcast 工具。
  - s13 团队协议：StageConfig.s13 同时包含 shutdown_request 与 plan_approval（合并了原 s09/s10 能力）。
  - 入口类：S13AgentTeams.main。
  - 参考路径：[StageConfig.java:533-571](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L181-L227)、[S13AgentTeams.java:1-24](file://src/main/java/com/learnclaudecode/agents/S13AgentTeams.java#L1-L24)
