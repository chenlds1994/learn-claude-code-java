...
## 项目结构
本项目采用按功能域划分的包结构：
- agents：Agent 运行阶段与编排（包含 s08 上下文压缩入口）
...
graph TB
subgraph "应用层"
S08["S08ContextCompact"]
Stage["StageConfig.s08()"]
end
subgraph "上下文压缩"
...
图表来源
- [S08ContextCompact.java:1-13](file://src/main/java/com/learnclaudecode/agents/S08ContextCompact.java#L1-L13)
...
- StageConfig.s08：启用压缩能力的阶段配置，暴露 compact 工具。
...
### 阶段配置与入口
- StageConfig.s08：启用 enableCompression，并暴露 compact 工具，使 Agent 可在对话中主动触发压缩。
- S08ContextCompact：启动 s08 阶段的示例入口，委托 Launcher 加载 StageConfig。
