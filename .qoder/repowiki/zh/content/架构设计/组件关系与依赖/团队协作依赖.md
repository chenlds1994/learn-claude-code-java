...
- common：JSON 序列化与工具
- agents：阶段配置与入口脚本（s13 演示多代理协作与协议）
...
- StageConfig：集中管理不同阶段的工具集与能力开关，驱动 s13 等多代理协作场景。
...
## 附录：使用示例
以下示例展示如何使用 s13 的能力进行多代理协作与协议交互。

- 启动多代理协作（s13）
  - 通过 S13AgentTeams.main 启动，启用 spawn_teammate、send_message、read_inbox、broadcast、shutdown_request、plan_approval 等团队协作全套工具。
  - Lead 可创建队友并向其发送消息；队友通过 read_inbox 拉取并处理。

- 团队协议（s13 内置）
  - s13 合并了原 s09/s10 的能力：Lead 可向队友发起关闭请求，队友通过 shutdown_response 回传结果；队友也可提交计划供 Lead 审批。
