...
## 项目结构
本项目采用分层与按能力组织的方式：
- agents：应用装配与阶段入口（如 S07、S11），通过 Launcher 统一启动；
- background：后台任务管理；
- skills：技能加载器；
...
- 各阶段入口（如 S07、S11）通过 Launcher 启动 Agent 运行时；
...
- 入口类 S07、S11 通过 Launcher 启动，体现阶段化学习路径。
...
### 后台任务创建与监控示例
- 入口：S11BackgroundTasks.main 通过 Launcher.launch 启动 StageConfig.s11；
...
### 技能动态加载示例
- 入口：S07SkillLoading.main 通过 Launcher.launch 启动 StageConfig.s07；
