...
- 注册阶段（配置期）：
  ...
  - 各阶段方法（s01..s17、sFull）按需叠加更多工具（permission_list/set、hook_*、todo、task、load_skill、compact、memory_*、background_*、schedule_cron 等、worktree_*、connect_mcp、workflow、goal_* 等）
...
Base --> Phase{"选择阶段(s01..s17/sFull)"}
Phase --> |s01| S01["仅 bash"]
Phase --> |s02| S02["bash + 文件工具"]
Phase --> |s03| S03["+ permission 权限"]
Phase --> |s04| S04["+ hook 钩子"]
Phase --> |s05| S05["+ todo"]
Phase --> |s06| S06["+ task(子代理)"]
Phase --> |s07| S07["+ load_skill"]
Phase --> |s08| S08["+ compact"]
Phase --> |s09| S09["+ memory 记忆"]
Phase --> |s10| S10["+ 任务管理工具"]
Phase --> |s11| S11["+ 后台任务工具"]
Phase --> |s12| S12["+ cron 定时调度"]
Phase --> |s13| S13["+ 团队全套(协作/协议/自治/worktree)"]
Phase --> |s14| S14["+ connect_mcp(MCP 插件)"]
Phase --> |s15| S15["全能力集成 Harness"]
Phase --> |s16| S16["+ workflow 工作流"]
Phase --> |s17| S17["+ goal 目标循环"]
S01 --> End(["就绪"])
S02 --> End
S03 --> End
S04 --> End
...
- 工具调用（运行时）
  - 工具名：字符串，如 bash、read_file、write_file、edit_file、permission_list、permission_set、hook_register、hook_list、hook_remove、todo、task、load_skill、compact、memory_add、memory_search、memory_delete、memory_list、background_run、check_background、schedule_cron、list_crons、cancel_cron、spawn_teammate、list_teammates、send_message、read_inbox、broadcast、shutdown_request、plan_approval、claim_task、idle、worktree_create、worktree_list、worktree_remove、worktree_events、connect_mcp（及动态发现的 mcp__server__tool）、workflow、goal_set、goal_status、goal_clear
...
- 常用工具一览（来自各阶段配置）
  ...
  - 权限（s03）：permission_list、permission_set
  - 钩子（s04）：hook_register、hook_list、hook_remove
  - 规划（s05）：todo
  - 子代理（s06）：task
  - 知识（s07）：load_skill
  - 上下文（s08）：compact
  - 记忆（s09）：memory_add、memory_search、memory_delete、memory_list
  - 任务（s10）：task_create、task_update、task_list、task_get
  - 后台（s11）：background_run、check_background
  - 定时（s12）：schedule_cron、list_crons、cancel_cron
  - 团队（s13）：spawn_teammate、list_teammates、send_message、read_inbox、broadcast、shutdown_request、plan_approval、shutdown_response、claim_task、idle、worktree_create、worktree_list、worktree_remove、worktree_events
  - MCP（s14）：connect_mcp（及动态发现的 mcp__server__tool）
  - 工作流（s16）：workflow
  - 目标（s17）：goal_set、goal_status、goal_clear
