...
## 简介
Learn Claude Code Java 是一个渐进式 AI Agent 学习框架，通过 17 个递进的学习阶段，从最基础的“Agent 循环”逐步演进到权限控制、钩子扩展、持久记忆、定时调度、MCP 插件、多代理协作、工作流编排与目标循环等复杂能力。后端基于 Java 17，提供交互式 REPL 运行环境与可扩展的工具调用机制；前端采用 Next.js（React + TypeScript），提供 Web 可视化界面、分步模拟器与各阶段架构图示，帮助初学者直观理解 Agent 的运行流程，同时为有经验的开发者提供清晰的技术细节与扩展点。

该项目的核心思想是：同一个 Agent 运行时，在不同阶段通过配置开关逐步启用更多能力（如工具集、权限控制、钩子扩展、Todo 规划、上下文压缩、持久记忆、后台任务、定时调度、团队通信、MCP 插件、Worktree 隔离、工作流编排、目标循环等），从而以最小认知成本掌握 Claude Code 风格的 Agent 设计与实现。
...
- 后端（Java）
  ...
  - 其他模块：tools、tasks、team、context、skills、background、permission、hooks、memory、scheduler、mcp、workflow、goal 等
- 前端（Next.js）
...
- 应用装配器
  - AppContext：集中创建并注入共享服务（命令工具、Todo、技能加载、上下文压缩、任务管理、后台任务、消息总线、队友管理、Worktree、权限管理、钩子、持久记忆、定时调度、MCP、工作流、目标循环等），最终组装 AgentRuntime
- 阶段配置
  - StageConfig：定义每个阶段的 system prompt、工具集、能力开关（Todo 提醒、权限、钩子、上下文压缩、记忆、后台任务、定时调度、收件箱、子代理可写、自治队友、MCP、工作流、目标循环等），并提供 s01-s17 与 sFull 的构建方法
- 运行时
...
- [AppContext.java:16-68](file://src/main/java/com/learnclaudecode/agents/AppContext.java#L16-L68)
- [StageConfig.java](file://src/main/java/com/learnclaudecode/agents/StageConfig.java)
- [AgentRuntime.java:23-498](file://src/main/java/com/learnclaudecode/agents/AgentRuntime.java#L23-L498)
...
C-->>R : "响应内容"
alt "tool_use"
R->>T : "执行工具bash/read/write/edit/todo/task/background/inbox/worktree/permission/hook/memory/cron/mcp/workflow/goal"
T-->>R : "工具结果"
...
### 阶段配置与能力演进
- 基础能力
  - s01：Agent 循环，最小闭环，仅 bash 工具
  - s02：工具调用，加入文件读写编辑工具
  - s03：权限系统（permission_list/set），工具调用前的授权管控
  - s04：钩子系统（hook_register/list/remove），生命周期事件扩展
  - s05：Todo 规划（todo），辅助多步骤任务规划
- 高级机制
  - s06：子代理（task），分治复杂问题
  - s07：技能加载（load_skill），动态注入领域知识
  - s08：上下文压缩（compact），解决长对话窗口限制
  - s09：记忆系统（memory_add/search/delete/list），跨会话持久记忆
  - s10：任务系统（task_create/update/list/get），结构化工作流
  - s11：后台任务（background_run/check），异步执行与状态查询
  - s12：定时调度（schedule_cron/list_crons/cancel_cron），周期性任务执行
- 专家能力
  - s13：多 Agent 协作（spawn_teammate/send_message/read_inbox/broadcast、shutdown_request/plan_approval、claim_task/idle、worktree_*），合并团队协议、自治队友与 Worktree 隔离
  - s14：MCP 插件（connect_mcp，及动态发现的 mcp__server__tool），运行时接入外部工具
  - s15：集成运行时（S15IntegratedHarness），打开 s01-s14 全部能力开关的综合 Harness
  - s16：工作流运行时（workflow），可恢复的多步流程编排
  - s17：目标循环（goal_set/status/clear），围绕可验证目标持续自主迭代
- 完整版本
  - sFull：合并上述能力，打开所有开关，形成完整视图
...
### 工具调度与执行
- 工具映射
  - 将模型返回的 tool_use 名称映射到本地 Java 实现（bash、read_file、write_file、edit_file、permission_list/set、hook_register/list/remove、todo、task、load_skill、compact、memory_add/search/delete/list、background_*、task_*、schedule_cron/list_crons/cancel_cron、spawn_teammate、send_message、read_inbox、broadcast、shutdown_request、plan_approval、claim_task、idle、worktree_*、connect_mcp 及 mcp__server__tool 动态工具、workflow、goal_set/status/clear）
- 执行策略
