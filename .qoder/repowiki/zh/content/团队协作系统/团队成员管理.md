...
- agents：StageConfig（阶段能力编排）、S13（示例入口）
...
- StageConfig：按教学阶段暴露不同能力（工具集），包括 s13 的团队与自治能力（合并了原 s09/s10/s11/s12 四个阶段）。
...
- 阶段能力
  - s13：一次性暴露完整团队协作工具集——spawn_teammate、list_teammates、send_message、read_inbox、broadcast、shutdown_request、plan_approval、claim_task、idle 与 worktree 系列工具（合并了原 s09/s10/s11/s12 四个阶段的能力）。
- 角色与权限
