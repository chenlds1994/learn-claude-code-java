...
- StageConfig：编排各教学阶段的工具与 system prompt，s07 引入 load_skill 工具并在 systemTemplate 中注入 ${SKILLS}。
- AppContext：装配 SkillLoader 并注入 AgentRuntime，使技能能力在运行时可用。
- S07SkillLoading / Launcher：演示入口，启动 s07 阶段以展示技能加载流程。
...
- s07 阶段新增 load_skill 工具，参数为 name，用于按需加载技能全文。
...
### 入口与演示：S07SkillLoading / Launcher
- S07SkillLoading.main 调用 Launcher.launch(StageConfig.s07())，启动具备技能加载能力的 Agent 示例。
...
### 工具绑定与配置选项
- 工具绑定：在 StageConfig.s07() 中新增 load_skill(name) 工具，供模型按需加载技能。
