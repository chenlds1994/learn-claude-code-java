...
## 项目结构
S06 子代理能力由以下关键部分组成：
- 入口类：S06Subagent 启动 s06 阶段
- 运行时：AgentRuntime 负责消息循环、工具调度与子代理执行
- 配置：StageConfig 定义 s06 的工具集与系统提示词
- 可视化与场景：前端可视化展示父子上下文隔离；JSON 描述典型交互流程与决策说明

```mermaid
graph TB
A["S06Subagent<br/>启动入口"] --> B["StageConfig.s06()<br/>定义工具与提示词"]
...
章节来源
- [S06Subagent.java:1-12](file://src/main/java/com/learnclaudecode/agents/S06Subagent.java#L1-L12)
- [StageConfig.java:115-122](file://src/main/java/com/learnclaudecode/agents/StageConfig.java#L115-L122)
...
## 核心组件
- S06Subagent：最小可运行的子代理演示入口，委托给 Launcher 并传入 s06 阶段配置。
- StageConfig：以“能力开关 + 工具集 + 系统提示词”的方式编排 s06，新增 task 工具用于委派子任务。
...
- 虽然当前 s06 未直接暴露“创建子代理”的方法，但通过 StageConfig 的 task 工具与 AgentRuntime 的 runSubagent，即可实现子代理能力。
...
## 结论
S06 子代理通过“独立上下文 + 工具集裁剪 + 固定轮次”的设计，实现了安全、可控的分治执行。父代理只需关注任务分解与结果汇总，子代理专注单一子任务，整体架构清晰、可扩展性强。
...
### 完整使用流程（从入口到执行）
- 启动：S06Subagent.main 调用 Launcher.launch(StageConfig.s06())。
- 配置：StageConfig.s06 添加 task 工具，设置系统提示词。
