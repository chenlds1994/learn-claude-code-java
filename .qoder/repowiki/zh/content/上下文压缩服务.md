...
## 项目结构
本项目将上下文压缩能力封装在 context 包中，并通过 agents 包的 s08 阶段配置暴露“compact”工具以触发手动压缩；同时通过 common 包提供工作区路径、JSON 序列化与环境配置等基础设施。前端可视化组件与标注文档用于教学演示与决策说明。

```mermaid
graph TB
subgraph "Agent运行时"
S08["S08ContextCompact<br/>启动示例"]
Stage["StageConfig.s08()<br/>启用压缩能力"]
end
subgraph "上下文压缩"
...
- StageConfig.s08()：开启压缩能力的阶段配置，暴露 compact 工具。
...
- 入口点：
  - S08ContextCompact 通过 Launcher 加载 StageConfig.s08()，启用压缩能力。
- 前端可视化与标注：
